                ...
                <v-data-table :headers="_headers" :items="orders" >
                    <template v-slot:item="row">
                        <tr class="row-in-order" >
                            ...
                            <td @click="goOrder(row.item._id)">{{ row.item.total_price | toVND }}</td>
                            <td @click="goOrder(row.item._id)">
                                <v-chip v-if="row.item.status == 'Pending'" class="ma-2"
                                        color="#E56A5E"
                                        dark
                                        label>
                                    {{ row.item.status }}
                                ...
                                <v-chip v-if="row.item.status == 'Approved'" class="ma-2"
                                        color="#00D17C"
                                        dark
                                        label>
                                    {{ row.item.status }}
                                ...
                                <v-chip v-if="row.item.status == 'Pick-up'" class="ma-2"
                                        color="#932FEA"
                                        dark
                                        label>
                                    {{ row.item.status }}
                                ...
                                <v-chip v-if="row.item.status == 'Delivering'" class="ma-2"
                                        color="#0FB7BA"
                                        dark
                                        label>
                                    {{ row.item.status }}
                                ...
                                <v-chip v-if="row.item.status == 'Received'" class="ma-2"
                                        color="#3b8bea"
                                        dark
                                        label>
                                    {{ row.item.status }}
                                ...
                                <v-chip v-if="row.item.status == 'Cancel'" class="ma-2"
                                        color="#92a4a4"
                                        dark
                                        label>
                                    {{ row.item.status }}
                            ...
                            <td @click="goOrder(row.item._id)">{{ row.item.date | toDateTime }}</td>

                            <td v-if="action">
                                <!-- <v-btn small dark color="#8C61FF"><v-icon left>mdi-eye</v-icon>View</v-btn> -->
                                <v-btn v-if="row.item.status == 'Pending'" 
                                        @click="confirm(row.item._id, 'Approved')"
                                        small dark color="#F62D51" 
                                ...
                                <v-btn v-if="row.item.status == 'Pending'"
                                        ...
                                        small dark color="#899878" 
                                        style="margin-left: 10px">
                                    <v-icon left>mdi-cancel</v-icon>
                                    Cancel
                                </v-btn>

                                <v-btn v-if="row.item.status == 'Pick-up'" 
                                        @click="confirm(row.item._id, 'Delivery')"
                                        small dark color="#EACE2A" 
                                        style="margin-left: 10px">
