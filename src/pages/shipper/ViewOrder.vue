                    ...
                    <v-col class="pa-2">
                        ...
                        <v-list-item two-line>
                            <v-list-item-content>
                                ...
                                <v-list-item-subtitle>{{ order._id | toCODE }}</v-list-item-subtitle>
                            </v-list-item-content>
                        </v-list-item>
                        ...
                        <v-list-item two-line>
                            <v-list-item-content>
                                ...
                                <v-list-item-subtitle>{{ order.total_price | toVND }}</v-list-item-subtitle>
                            </v-list-item-content>
                        </v-list-item>
                        ...
                        <v-list-item two-line>
                            <v-list-item-content>
                                ...
                                <v-list-item-subtitle>{{ order.date | toDateTime }}</v-list-item-subtitle>
                            </v-list-item-content>
                        </v-list-item>
                    </v-col>
                    ...
                    <v-col v-if="order.shipper" class="pa-2" >
                        ...
                        <v-list-item two-line>
                            <v-list-item-content>
                                ...
                                <v-list-item-subtitle>{{ order.shipper.username }}</v-list-item-subtitle>
                            </v-list-item-content>
                        </v-list-item>
                        <v-list-item two-line>
                            <v-list-item-content>
                                <v-list-item-title>Fullname:</v-list-item-title>
                                <v-list-item-subtitle>{{ order.shipper.full_name }}</v-list-item-subtitle>
                            </v-list-item-content>
                        </v-list-item>
                        <v-list-item two-line>
                            <v-list-item-content>
                                <v-list-item-title>Phone:</v-list-item-title>
                                <v-list-item-subtitle>{{ order.shipper.phone }}</v-list-item-subtitle>
                            </v-list-item-content>
                        </v-list-item>
                    </v-col>
                </v-card>
                    ...
                    <v-col class="pa-2" >
                        ...
                        <v-simple-table>
                            <template v-slot:default>
                                ...
                                <tbody>
                                    <tr v-for="(item, i) in order.orderItems"
                                        :key="item._id">
                                        <td>{{ i+1 }}</td>
                                        <td align="center">
                                            <img class="image-product-in-table" 
                    ...
                    <div class="text-right p-3">
                        <v-list-item>
                            <v-list-item-content>
                                ...
                                <v-list-item-subtitle style="font-size: 1.75rem; font-weight: 700">{{ order.total_price | toVND }}</v-list-item-subtitle>
                            </v-list-item-content>
                        </v-list-item>
                    </div>
                </v-card>
        ...
        <v-container class="grey lighten-5">
            <v-row class="mb-6 p-3" no-gutters data-app style="justify-content: center !important">
                ...
                <v-col v-if="order.status == 'Approved'" md="auto">
                    <v-btn @click="confirm(order._id, 'Pick-up')"
                            color="#EACE2A">
                ...
                <v-col v-if="order.status == 'Pick-up'" md="auto">
                    <v-btn @click="confirm(order._id, 'NO Pick-up')"
                            dark color="#F44336">
...
export default {
    ...
    methods: {
        async getOrderByID() {
            await axios.get(`http://localhost:5000/api/orders/${this.$route.params.order_id}`)
                .then((res) => {
                    this.order = res.data;
                })
                .catch((err) => {
        ...
        async pickup() {
            ...
            await axios.get(`http://localhost:5000/api/orders/pickup/${this.id_selected}`, config)
                        .then((res) => {
                            console.log(res);
                            this.getOrderByID();
        ...
        async cancelPickup() {
            ...
            await axios.get(`http://localhost:5000/api/orders/cancelPickup/${this.id_selected}`, config)
                        .then((res) => {
                            console.log(res);
                            this.getOrderByID();
                            this.visibleDialog = false;
...
<style scoped>
    ...
    .btn-approved i, .btn-back i, .btn-cancel-order i {
        padding-right: 8px;
    }
