<template>
  <div class="container-fluid pa-6">
    <v-row align="center">
      <h3 class="ma-2">Update post</h3>
    </v-row>
    <v-row>
      <v-card width="100%" class="ma-2 py-4" flat>
        <v-text-field ref="title" v-model="post.title" :rules="[() => !!post.title || 'This field is required']"
          :error-messages="errorMessages" label="Title" placeholder="Post's Title" outlined required></v-text-field>
...
export default {
  ...
  async created() {
    ...
    await PostAPI.getById(this.$route.params.post_id)
      .then((res) => {
        this.post = res.data;
  ...
  methods: {
      ...
      if (!this.hasEmptyField()) {
        await uploadFileToCloudinary(this.post.thumbnail, "blogs").then(
          (fileResponse) => {
            ...
            // send API
            PostAPI.update(this.post._id, this.post, config)
              .then((res) => {
    ...
    hasEmptyField() {
      ...
      if (this.post.title == "" || this.post.content == "" || this.post.thumbnail == "") {
