                    ...
                    <table class="table table-bordered"
                        ...
                        <tbody>
                            <template v-for="user in data">
                                <tr :key="user._id">
                                    <td>
                                        <img v-if="user.avatar == ''" class="avatar-in-table" 
                                            src="../../../public/assets/assets/img/account-image-placeholder.jpg"/>
                                        
                                        <v-img v-else class="avatar-in-table" 
                                                :src="`/avatar/${user.avatar}`"></v-img>
                                    </td>
                                    <td>{{ user.username }}</td>
                                    <td>{{ user.full_name }}</td>

                                    <td>
                                        <p v-if="user.position == 'Admin'" class="user-role role-admin">{{ user.position }}</p>
                                        <p v-if="user.position == 'customer'" class="user-role role-customer">{{ user.position }}</p>
                                        <p v-if="user.position == 'shipper'" class="user-role role-shipper">{{ user.position }}</p>
                                    </td>

                                    <td>
                                        <p v-if="user.status == 'active'" class="user-status stt-active">{{ user.status }}</p>
                                        <p v-if="user.status == 'disable'" class="user-status stt-disable">{{ user.status }}</p>
                                    </td>
                                    <td>
                                        <div class="actions-in-table">
                                            ...
                                            <button v-if="user.status == 'active'" class="btn-in-table btn-disable"
                                                    data-toggle="modal"
                                            ...
                                            <button v-if="user.status == 'disable'" class="btn-in-table btn-active"
                                                    data-toggle="modal"
                                                    data-target="#exampleModalCenter"
                        ...
                        <div class="modal-dialog modal-dialog-centered" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLongTitle"> {{ this.dataModal.title }} </h5>
                                    <button type="button"
                                        class="close"
                                        data-dismiss="modal"
                                        aria-label="Close">
...
export default {
    ...
    methods: {
        ...
        async updateStatus() {
            ...
            this.msg_snackbar = "";
            await axios
                .patch(`http://localhost:5000/api/updateUserStatus/${this.id_selected}`,
